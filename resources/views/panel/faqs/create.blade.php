@extends('panel.layout.master' , ['title' => 'إضافة سؤال شائع'])
    ...
    <div class="kt-subheader  kt-grid__item" id="kt_subheader">
        <div class="kt-container  kt-container--fluid ">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">إضافة سؤال شائع</h3>
            </div>
        </div>
    </div>
                        ...
                        @foreach(locales() as $key=>$language)
                            ...
                            <div class="tab-pane {{$i==1?'active':''}}" id="kt_tabs_{{$i}}" role="tabpanel">
                                ...
                                <div class="form-group">
                                    <label>السؤال</label>
                                    <input class="form-control m-input" type="text" name="{{ 'question_'.$key }}"
                                           placeholder="السؤال" required value="{{ isset($item) ? $item->getTranslation($key)->question : "" }}">

                                </div>
                                <div class="form-group">
                                    <label>الإجابة</label>
                                    <textarea class="form-control" rows="5"
                                              name="{{ 'answer_'.$key }}" required placeholder="الإجابة">{{ isset($item) ? $item->getTranslation($key)->answer : ""  }}</textarea>
                                </div>
